...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep  9 08:46:45 2021
@author: Anonymous
"""
...
class FactorModel:
    ...
    def get_logits_from_sample(
        ...
        loading_mat = self.get_loadings(
            self.cur_rotation, sort=sort, sort_by_flowprobs=True
        )
        w = self.w
        h = self.h
    ...
    # *** rotations ***
    @np.deprecate
    def _dvarimax(self, loading_mat_org, init_rot_mat=None, **kwargs):
        assert self.num_logits == 2, "Only implemented for exactly 2 classes."
        ...
        diff = rearrange(diff[:, :, 1, :] -
                         diff[:, :, 0, :], "w h r -> (w h) r")

        init = ortho_group.rvs(dim=self.rank)
        _, rot_mat = self.rotator._varimax(diff, init=init)
        loading_mat = np.dot(loading_mat_org, rot_mat)

        return loading_mat, None
    ...
    def _pca(self, loading_mat_org, init_rot_mat=None, **kwargs):
        sqfactor = np.dot(loading_mat_org.T, loading_mat_org)
        w, v = np.linalg.eig(sqfactor)
        loading_mat = np.dot(loading_mat_org, v)
        return loading_mat, v, None

    @np.deprecate
    def _proj(self, loading_mat_org, mask, secondary="varimax", **kwargs):
        # https://gist.github.com/iizukak/1287876/edad3c337844fac34f7e56ec09f9cb27d4907cc7
        # Q, R = np.linalg.qr(X) take Q

        proj_coeff = np.dot(loading_mat_org.T, mask)
        o_mask = np.eye(self.rank)
        o_mask[:, 0] = proj_coeff / np.linalg.norm(proj_coeff)
        o_mask = gram_schmidt(o_mask)

        rotated = np.dot(loading_mat_org, o_mask)

        # ToDo: What is this fix factor for?
        fix_factor = rotated[:, 0]
    ...
    def plot_samples_stepwise_factor(
        ...
        """inspect samples from steps for a single component of a given
        factor model"""
    ...
    def plots_statistics(self, filename=None, show=False):
        if (
    ...
    def get_uncertainty_image(self, flowprobs, upscale=2):
        ...
        if self.neutralcolor == (1, 1, 1):
            return img

        # TODO: what is this for? For black this does not change anything, for weight it makes everything brighter?
        # for x in range(3):
        #    img[:, :, x] = img[:, :, x] + (1 - weights) * self.neutralcolor[x]
        return img
...
# TODO: This should be made the main method and callable with args from console
